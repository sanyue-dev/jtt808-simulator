...
- 坐标系：项目直接使用百度地图坐标（BD09），平台端显示时需注意坐标转换

### Web Layer（cn.org.hentai.simulator.web）

Spring Boot + MyBatis + FreeMarker 模板，前端基于 jQuery/Bootstrap + 百度地图 API。
...
使用 `org.yzh:jtt808-protocol` 库处理 JTT808 编解码。支持的消息类型包括 T0100（注册）、T0102（鉴权）、T0001（终端通用应答）、T0200（位置上报）、T8100（注册应答）、T8300（文本下发）等。位置上报时会将 BD09 坐标转换为 WGS84。
...
- 配置集中在 `application.yml`，包括数据库、地图 API Key、车辆服务器地址
